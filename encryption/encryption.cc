...
// Function to write vector content to a file
void write_file1(const std::string &file_path, std::string content) {
    std::ofstream file(file_path, std::ios::binary);
    if (!file) {
        throw std::runtime_error("Failed to open file for writing");
    }
...
int main() {
    try {
        ...
        write_file1(pubKey_path, public_key);
        std::string privKey_path = "privKey.pem";
        write_file1(privKey_path, private_key);
